{% from 'docker_prometheus/map.jinja' import config with context %}
...
{{ app.name }}_{{ file.name }}:
  ...
  - template: jinja
  - contents_pillar: docker_prometheus:{{ app.name }}:supporting_files:{{ loop.index0 }}:contents
{% endfor %}
...
{{ app.location }}/docker-compose.yml:
  ...
  - dir_mode: 755
  - mode: 744
  - contents_pillar: docker_prometheus:{{app.name}}:docker_compose_yaml

{{ app.location }}/.env:
  file.managed:
  - source: salt://docker_prometheus/files/env_file.jinja2
  - template: jinja
  - makedirs: True
  - replace: True
  - dir_mode: 755
...
/etc/systemd/system/{{ app.name }}.service:
  file.managed:
  - source:
    - salt://docker_prometheus/files/app.service.jinja2
  - template: jinja
  - replace: True
  - context:
      app: {{ app }}
